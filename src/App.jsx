...
const modelSteps = [
  {
    number: "01",
    title: "Identify the right patients",
    description:
...
function Logo() {
  return (
    <a href="#top" className="flex min-w-0 items-center gap-3" aria-label="The Health Watchers home">
      <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#0F4C5C] shadow-sm">
        <Activity className="h-5 w-5 text-white" strokeWidth={2.2} />
      </div>
      <div className="min-w-0 leading-tight">
        <div className="truncate text-sm font-semibold tracking-[-0.02em] text-slate-950 sm:text-base">The Health Watchers</div>
        <div className="hidden text-xs font-medium tracking-[0.14em] text-slate-500 uppercase sm:block">In-Home Medical Care</div>
      </div>
    </a>
  );
}
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            ...
            <a
              href="#contact"
              className="inline-flex items-center gap-2 rounded-full bg-[#0F4C5C] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#123F4B]"
            >
              Partner With Us
        ...
        <div className="fixed inset-0 z-[9999] bg-slate-950/55 lg:hidden" role="dialog" aria-modal="true" aria-label="Mobile navigation">
          ...
          <div className="absolute right-0 top-0 z-10 flex h-screen max-h-screen w-full max-w-[22rem] flex-col overflow-y-auto overscroll-contain bg-white p-5 shadow-2xl sm:p-6">
            ...
            <nav className="mt-8 grid shrink-0 gap-2" aria-label="Mobile navigation">
              {navItems.map((item) => (
                <a
                  ...
                  href={item.href}
                  onClick={() => setOpen(false)}
                  className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-base font-semibold text-slate-900 transition hover:bg-slate-50"
                >
                  {item.label}
                </a>
              ))}
            ...
            <a
              href="#contact"
              onClick={() => setOpen(false)}
              className="mt-6 inline-flex w-full shrink-0 items-center justify-center gap-2 rounded-full bg-[#0F4C5C] px-5 py-3 text-sm font-semibold text-white shadow-sm"
            >
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-5 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-28">
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <a
              href="#contact"
              className="inline-flex items-center justify-center gap-2 rounded-full bg-[#0F4C5C] px-6 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#123F4B]"
            >
              Partner With Us
              <ArrowRight className="h-4 w-4" />
            </a>
            <a
              href="#services"
              className="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-6 py-3.5 text-sm font-semibold text-slate-900 shadow-sm transition hover:border-slate-400"
            >
              Explore Our Model
            </a>
          </div>
  ...
  const careOptions = [
    {
      ...
      title: "Telemedicine support",
      description:
        "Medical professionals can connect virtually with patients, residents, facility teams, and care partners to help assess needs, support follow-up, and determine the appropriate next step.",
    },
    {
      icon: Stethoscope,
      title: "In-house or onsite visits",
      description:
        "Where appropriate and available, our clinical team can support in-person visits in the home, facility, or senior living setting to bring care closer to the patient.",
    },
    {
      icon: Users,
      title: "Coordinated care team model",
      description:
        "Physicians, advanced practice providers, nurses, care coordinators, and patient engagement specialists work together to support proactive care management workflows.",
    },
  ];
        ...
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <div className="grid gap-5">
            {careOptions.map((option) => {
              ...
              return (
                <div key={option.title} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex gap-5">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#E8F4F1] text-[#0F4C5C]">
                      <Icon className="h-6 w-6" />
                    </div>
                    <div>
                      <h3 className="text-xl font-semibold tracking-[-0.03em] text-slate-950">{option.title}</h3>
    ...
    <section id="care-model" className="bg-white py-24">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-5">
          {modelSteps.map((step) => (
            <div key={step.number} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-8 text-sm font-semibold tracking-[0.16em] text-[#0F766E]">{step.number}</div>
              <h3 className="text-lg font-semibold tracking-[-0.03em] text-slate-950">{step.title}</h3>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20">
          <div className="rounded-[1.5rem] bg-white p-5 text-slate-950">
            ...
            <div className="mt-6 space-y-4">
              {[
                { label: "Completed visits", width: "86%" },
                { label: "Virtual follow-ups", width: "72%" },
                { label: "Care gaps addressed", width: "64%" },
                { label: "Documentation completion", width: "91%" },
              ].map((item) => (
                <div key={item.label}>
                  <div className="mb-2 flex justify-between text-sm">
                    <span className="font-medium text-slate-700">{item.label}</span>
                    <span className="font-semibold text-slate-950">{item.width}</span>
                  </div>
                  <div className="h-2 rounded-full bg-slate-100">
                    <div className="h-2 rounded-full bg-[#0F4C5C]" style={{ width: item.width }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <form className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70 lg:p-8" aria-label="Partnership inquiry form">
          <div className="grid gap-5 sm:grid-cols-2">
            <label className="grid gap-2">
              ...
              <input name="first-name" autoComplete="given-name" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm font-semibold text-slate-700">Last name</span>
              <input name="last-name" autoComplete="family-name" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
            <label className="grid gap-2 sm:col-span-2">
              <span className="text-sm font-semibold text-slate-700">Organization name</span>
              <input name="organization" autoComplete="organization" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm font-semibold text-slate-700">Email</span>
              <input name="email" type="email" autoComplete="email" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
            <label className="grid gap-2">
              <span className="text-sm font-semibold text-slate-700">Phone</span>
              <input name="phone" type="tel" autoComplete="tel" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
            <label className="grid gap-2 sm:col-span-2">
              <span className="text-sm font-semibold text-slate-700">Organization type</span>
              <select name="organization-type" className="h-12 rounded-2xl border border-slate-200 px-4 text-sm outline-none transition focus:border-[#0F4C5C]">
                <option>Select one</option>
                <option>Skilled Nursing Facility</option>
                <option>Assisted Living Community</option>
                <option>Independent Living Community</option>
                <option>Continuing Care Retirement Community</option>
                <option>Physician Group</option>
                <option>ACO / MSO / Population Health</option>
                <option>Other</option>
              </select>
            </label>
            <label className="grid gap-2 sm:col-span-2">
              <span className="text-sm font-semibold text-slate-700">How can we help?</span>
              <textarea name="message" rows={5} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#0F4C5C]" />
            </label>
          </div>
  ...
  return (
    <footer className="bg-slate-950 py-12 text-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-5 lg:flex-row lg:items-center lg:justify-between lg:px-8">
        <div>
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-[#0F4C5C]">
              <Activity className="h-5 w-5" />
            </div>
            <div>
              <div className="font-semibold tracking-[-0.02em]">The Health Watchers</div>
              <div className="text-xs font-medium tracking-[0.14em] text-slate-400 uppercase">In-Home Medical Care</div>
            </div>
          </div>
